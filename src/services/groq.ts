      ...
      messages: [
        {
          role: 'system',
          content:
            'Translate the following text to Arabic. Preserve the original meaning and tone exactly. Return only the Arabic translation — no explanations, no notes, no extra text.',
        },
...
export const generateGroqSuggestion = async (
  fieldName: string,
  ...
  if (!apiKey) {
    throw Object.assign(new Error('No Groq API key'), { code: 'NO_API_KEY' });
  }
  ...
  try {
    ...
    if (e?.status === 401) {
      throw Object.assign(new Error('Invalid Groq key'), { code: 'INVALID_KEY' });
    }
    if (e?.status === 429) {
      throw Object.assign(new Error('Rate limit'), { code: 'RATE_LIMIT' });
    }
    throw Object.assign(new Error((e?.message as string) || 'Request failed'), { code: 'API_ERROR' });
  }
};
