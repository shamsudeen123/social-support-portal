...
export const generateAISuggestion = async (
  ...
  if (!apiKey) {
    throw Object.assign(new Error('No API key'), { code: 'NO_API_KEY' });
  }
  ...
  try {
    ...
    if (e?.status === 401) {
      throw Object.assign(new Error('Invalid key'), { code: 'INVALID_KEY' });
    }
    if (e?.status === 429) {
      const isQuota = (e?.error as Record<string, unknown>)?.code === 'insufficient_quota';
      throw Object.assign(
        new Error(isQuota ? 'Quota exceeded' : 'Rate limit'),
