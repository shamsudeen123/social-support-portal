...
const SectionHeader = ({ icon: Icon, title, color, lang }: { icon: React.ElementType; title: string; color: string; lang: string }) => (
...
const DetailRow = ({ label, value }: { label: string; value: string }) => (
  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 1.5, py: 0.6 }}>
    ...
    <Typography variant="caption" color="text.primary" sx={{ fontWeight: 600, textAlign: 'end', wordBreak: 'break-word' }}>
      {value || '—'}
    </Typography>
  </Box>
);

const SituationCard = ({ label, value, color, isDark, lang, loading }: { label: string; value: string; color: string; isDark: boolean; lang: string; loading?: boolean }) => {
  return (
    <Box sx={{
      p: 1.75,
      borderRadius: '10px',
      ...
      <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700, display: 'block', mb: 0.75, letterSpacing: lang === 'ar' ? 0 : 0.3 }}>
        {label}
      </Typography>
      {loading ? (
...
const SuccessPage = () => {
  ...
  const { referenceNumber, formData } = useSelector((s: RootState) => s.form);
  const isDark = theme.palette.mode === 'dark';
  const lang = i18n.language;
  const n = (num: number | string): string => numeral(num, lang);

  const [arContent, setArContent] = useState<ArContent | null>(null);
  const [translating, setTranslating] = useState<boolean>(false);
  const didTranslate = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    Promise.all(values.map(txt => txt?.trim() ? translateToArabic(txt) : Promise.resolve(txt || '')))
      .then(translated => {
        ...
        setArContent(result as unknown as ArContent);
  ...
  const genderMap: Record<string, string> = { male: t('personal.male'), female: t('personal.female'), other: t('personal.other'), prefer_not: t('personal.preferNotToSay') };
  const maritalMap: Record<string, string> = { single: t('family.single'), married: t('family.married'), divorced: t('family.divorced'), widowed: t('family.widowed') };
  const employmentMap: Record<string, string> = { employed: t('family.employed'), unemployed: t('family.unemployed'), self_employed: t('family.selfEmployed'), student: t('family.student'), retired: t('family.retired'), unable: t('family.unable') };
  const housingMap: Record<string, string> = { owned: t('family.owned'), rented: t('family.rented'), living_with_family: t('family.livingWithFamily'), homeless: t('family.homeless'), other: t('family.other') };

  const fullAddress = [formData.address, formData.city, formData.state, formData.country].filter(Boolean).join(', ');

  return (
    <Box sx={{
      flex: { sm: 1 }, overflowY: { sm: 'auto' },
      py: { xs: 1.5, sm: 2.5 },
      ...
      <Paper elevation={0} sx={{
        ...
        <Box sx={{ px: { xs: 3, sm: 4 }, pt: { xs: 3, sm: 3.5 }, pb: 2.5, textAlign: 'center' }}>
          ...
          <Typography variant="h6" color="success.main" gutterBottom sx={{ fontWeight: 800 }}>
            {t('submit.title')}
          </Typography>
          ...
          <Box sx={{
            ...
            borderRadius: '12px', px: { xs: 2.5, sm: 4 }, py: 1.75,
          }}>
            <Typography variant="caption" color="text.secondary"
              sx={{ fontWeight: 700, textTransform: lang === 'ar' ? 'none' : 'uppercase', letterSpacing: lang === 'ar' ? 0 : 1.2 }}>
              {t('submit.referenceNumber')}
            </Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mt: 0.5 }}>
              <TagIcon sx={{ fontSize: 18, color: 'primary.main' }} />
              <Typography variant="h6" color="primary.main" sx={{ fontWeight: 800, letterSpacing: lang === 'ar' ? 0 : 1.5 }}>
                {displayRef}
              </Typography>
            </Box>
        ...
        <Divider sx={{ mx: { xs: 3, sm: 4 } }}>
          <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700, letterSpacing: lang === 'ar' ? 0 : 0.5 }}>
